...
async function ensureSeed() {
  ...
  if (adminAccount) return; // Already seeded
...
async function getFullStateFromDB() {
  ...
  const settings = {
    ...
    totalBuses: clampTotalBuses(settingsDoc.batas_maksimal_armada ?? 6),
...
export async function writeStore(nextState) {
  ...
  const settings = {
    ...nextState.settings,
    totalBuses: clampTotalBuses(nextState.settings?.totalBuses ?? 6)
  };
  const fleetOptions = getFleetOptions(settings.totalBuses);
  let buses = Array.isArray(nextState.buses) ? nextState.buses : [];
  ...
  buses = fleetOptions.map((number) => {
    ...
    return {
      number,
      lat: isTracking ? Number(existing?.lat ?? busDefaultPosition.lat) : busDefaultPosition.lat,
      lng: isTracking ? Number(existing?.lng ?? busDefaultPosition.lng) : busDefaultPosition.lng,
      updatedAt: existing?.updatedAt || new Date().toISOString(),
      isTracking,
      driverId: existing?.driverId || null,
      stationName: existing?.stationName || settings.chargingStationName,
      statusText: existing?.statusText || (isTracking ? `Bilis ${number} sedang dalam perjalanan.` : `Bilis ${number} terparkir di ${settings.chargingStationName}.`)
    };
  });
